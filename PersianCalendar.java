...
package ir.vahidzazadeh.utility;
 ...
 * persian date with two methods
 *
 * @author vahid rezazadeh (vahid.rezazadeh1372@gmail.com)
 */
...
public class PersianCalendar implements Serializable {
    ...
    private static String jalali_month_names[] = {"فروردین", "اردیبهشت", "خرداد",
     ...
     * Converts specified gregorian date to persian date in form of (yyyy/mm/dd)
     */
    public static String[] getPersianDate2(int gregYear, int gregMonth, int gregDay) {
        // passed days from Greg orig
        double d = Math.ceil((gregYear - 1) * greg_len);
        // passed days from jalali base
        double d_j = d + greg_origin_from_jalali_base
        ...
        // first result! jalali year
        double j_y =(int) Math.ceil(d_j / len) - 2346;
        // day of the year
        double j_days_of_year = (int)Math
                .floor(((d_j / len) - Math.floor(d_j / len)) * 365) + 1;


        String str[] = {(int)j_y + "",(int) month(j_days_of_year) + "", (int)dayOfMonth(j_days_of_year) + ""};
     ...
     * Converts specified gregorian date to persian date in form of (yyyy/mm/dd)
     */
    public static String getPersianDateString(Date d) {
        GregorianCalendar gc = new GregorianCalendar();
        gc.setTime(d);
        int year = gc.get(Calendar.YEAR);
        String[] str = getPersianDate2(year, (gc.get(Calendar.MONTH)) + 1,
                gc.get(Calendar.DAY_OF_MONTH));

          return String.format("%s %s %s",str[2],jalali_month_names[Integer.parseInt(str[1])-1],str[0]);
